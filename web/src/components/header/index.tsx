...
import Transition from "../../new-components/Transition";
import ichrp from "../../images/ichrp.png";
import { useWindowScroll, useClickAway } from "react-use";
import { FaRegEnvelope, FaQuestion, FaRegCalendarAlt, FaBook } from "react-icons/fa";
import { useHotkeys } from "react-hotkeys-hook";

// px before the header turns white
  ...
  return (
    <div>
      <button
        ...
        onClick={open}
        className={`group inline-flex items-center mx-2 border-b-2 px-4 py-2 text-base leading-6 font-medium focus:outline-none
          ${
            y > 10
      ...
      >
        <span className={shouldShow ? (y > 10 ? "text-gray-500" : "text-gray-200") : ""}>
          {name}
        </span>
...
const Header = ({ showNav, siteTitle, scrolled, navMenuItems = [], textWhite = true }) => {
  ...
  const { y } = useWindowScroll();
  const isHeaderLight = y > SCROLL_THRESHOLD;
  ...
  return (
    <div className="fixed top-0 z-10 w-full">
      <div
        ...
        style={{ "--bg-opacity": isHeaderLight ? 1 : 0 }}
      >
        <div className="container px-3 sm:px-6 m-auto flex w-full lg:grid grid-cols-3 justify-between items-center border-b-1 border-gray-100 py-4 ">
          <Link to="/" className="flex">
            <img className="max-h-10" src={ichrp} alt="Workflow" />
            <span
              className={`hidden md:flex ml-2 font-extrabold text-2xl italic ${
                isHeaderLight ? "text-gray-700 " : "text-gray-100"
              }`}
            >
              Set Them Free
            </span>
          ...
          <MobileMenu />
          <nav className="hidden md:flex justify-center self-center">
            <MenuItem
              name="Learn"
              items={[
                {
          ...
          <div className="hidden md:flex items-center space-x-8 justify-end ml-8">
